...
export default function Dashboard() {
  ...
  //
  //   try {
  //     // Make a POST request to your backend to add a new player
  //     const response = await axiosInstance.post("/addPlayer", formData);
  //
  //     if (response.status === 201) {
  //       // On success, add the new player to the state (to re-render the list)
  //       setPlayers(prevPlayers => [...prevPlayers, response.data.player]);
  //       setMessage("Player added successfully!");
  //       setFormData({});  // Clear the form after submission
  //     }
  //   } catch (error) {
  //     setMessage(error.response.data || "Error adding player");
  //   }
  // };
  ...
  const handlePlayerSubmit = async (e) => {
    ...
    // **Add this code to log the form data**
